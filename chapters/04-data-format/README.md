...
「サーバーが何を返してきたか」「ブラウザや JS がそれをどう解釈するか」を担当するのが `Content-Type` ヘッダーと、そこに書かれた **MIME タイプ** です。この章は「ボディの中身は何で、どう読むのか」を扱います。
...
- `<meta charset="utf-8" />` は **文字コード宣言**。これがないと文字化けする
...
HTML はブラウザがかなり頑張って解釈してくれる、という設計です。閉じタグを忘れても、属性のクオートを外しても、それなりに表示されてしまうことが多い。これは初心者には親切ですが、ちゃんと書かれていないと **JavaScript で操作するときにバグる** ので、書くときは省略しないのが無難です。
...
- 文字列（`"hello"`）— **ダブルクオート必須**。シングルクオートは不可
- 数値（`42`, `3.14`）— クオートしない
- 真偽値（`true`, `false`）— 小文字
- `null` — 小文字
- 配列（`[1, 2, 3]`）
...
この実用面の良さで、API のレスポンス形式は 2010 年代以降ほぼ JSON が標準になりました。
...
`#` でも `//` でも `/* */` でもダメ。仕様で禁止されています。設定ファイルとして使うときに困りがちで、これを嫌って **JSONC**（コメント可能な JSON、VS Code が使う）や **YAML**、**TOML** といった派生・代替が生まれました。
...
JSON は **`application/json`** で、`text/json` ではないので注意。

#### `charset`

`Content-Type: text/html; charset=utf-8` のように、後ろに **文字コード** を添えられます。これがないとブラウザが文字コードを推測することになり、文字化けの原因になります。
...
OS（とくに Windows）はファイル拡張子で種類を判断しますが、Web の通信ではファイル拡張子に頼らず `Content-Type` で判断するのが原則。URL の末尾が `.json` でも、サーバーが `Content-Type: text/html` で返してきたらブラウザは HTML として解釈します（そして表示が壊れる）。
...
今では「文字コード＝とりあえず UTF-8」が世界の合意事項。HTML5 の標準も UTF-8 です。

#### 文字化けが起きる理由

文字化けは **「保存したときの文字コードと、読むときの文字コードがズレている」** ときに起きます。
...
ブラウザが推測するので最近はあまり化けませんが、**API が UTF-8 で返してきた JSON を、`charset=shift_jis` で誤って解釈する** とアプリ側で派手に化けます。
...
**BOM（Byte Order Mark）** はファイル先頭に付ける 3 バイトの目印（`EF BB BF`）。「これは UTF-8 です」と宣言する役割。Excel 対策で付けることがあるが、付けるとプログラムが「最初の文字が変な空白に見える」とハマることもある。
...
サーバーは **テキストを送るだけ**。「これは JSON だよ」と教えているのは `Content-Type` ヘッダー。クライアントはそれを信じてパースする。だから `Content-Type` を間違えると全てが壊れる。
...
- **JSON のクオートはダブル必須**: `{'name': 'Alice'}` は **Python の dict としてはアリ** だが JSON としては不正。`json.dumps()` を通すと自動で正しい JSON になる
- **JSON にコメントは書けない**: 設定ファイルに使いたくなると詰まる。YAML / TOML / JSONC を検討する
- **CSV のエンコーディング**: 日本語 CSV は Shift_JIS と UTF-8 の両方が世の中に流れていて事故りやすい。`open()` の `encoding=` を意識する
...
- **拡張子と MIME は別**: 拡張子 `.json` のファイルでも、サーバーが `text/html` で返したらブラウザは HTML として扱う
